				    	...
				    	<c:choose>
				    		<c:when test="${activity.menSdate >= now}">
					         ...
					         <c:when test="${activity.menSdate <= now and activity.menEdate >= now }">
					         	<button type="button" id="modifyBtn" onclick="location.href='mentoringModifyStatus.do?supNo=${support.supNo}';" class="btn btn-warning btn-pills waves-effect waves-themed">중도포기</button>
					         </c:when>
